...
// Kamus untuk menerjemahkan URL ke nama Seksi asli di Database
...
const DataSeksi: React.FC = () => {
  ...
  const exactSeksiName = seksiMap[seksiParam || ''] || '';
  ...
  // Fetch ulang setiap kali URL seksi berubah
  useEffect(() => {
    fetchMembers();
    setSearchTerm(''); 
    setCurrentPage(1); 
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        // FILTER: Cocokkan member.seksi dengan exactSeksiName
        const filteredData = allData.filter(
          (member) => member.seksi === exactSeksiName
        );
  ...
  return (
    <>
      <PageBreadcrumb pageTitle={`Data Unsur ${exactSeksiName}`} />
        ...
        <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-6 shadow-default dark:border-gray-800 dark:bg-gray-900 sm:px-7.5">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
              <h4 className="text-xl font-semibold text-gray-900 dark:text-white">
                Seksi / Unsur: <span className="text-primary">{exactSeksiName}</span>
              </h4>
          ...
          <div className="mb-6">
            <div className="relative w-full max-w-md">
              ...
              <input
                type="text"
                placeholder={`Cari jemaat di unsur ${exactSeksiName}...`}
                value={searchTerm}
          ...
          <div className="flex flex-col">
            <div className="grid grid-cols-3 rounded-sm bg-gray-100 dark:bg-gray-800 sm:grid-cols-6 border border-gray-200 dark:border-gray-700">
              ...
              <div className="p-2.5 text-center xl:p-5"><h5 className="text-sm font-medium uppercase text-gray-900 dark:text-gray-200">Kelompok Doa</h5></div>
            ...
            {loading ? (
              ...
              <div className="p-5 text-center text-gray-500 border border-t-0 border-gray-200 dark:border-gray-700">
                {searchTerm ? 'Data tidak ditemukan.' : `Belum ada data jemaat di unsur ${exactSeksiName}.`}
              </div>
            ) : (
              currentData.map((member, key) => (
                <div className={`grid grid-cols-3 sm:grid-cols-6 items-center border border-t-0 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition`} key={member.id}>
                  ...
                  {/* Karena kita sudah di halaman Seksi, lebih informatif jika kita tampilkan Kelompok Doa-nya di sini */}
                  <div className="flex items-center justify-center p-2.5 xl:p-5">
                    <p className="text-sm text-gray-600 dark:text-gray-400 text-center">{member.kelompok_doa}</p>
                  </div>
